...
func (c *Component) mergeTreeEngine(table, variant string, args ...sb.Expr) sb.Engine {
	if c.d.ClickHouse.ClusterName() != "" {
		zkPath := fmt.Sprintf("/clickhouse/tables/shard-{shard}/%s%s", table, c.d.ClickHouse.ZooKeeperSuffix())
		if helpers.Testing() {
			zkPath = fmt.Sprintf("/clickhouse/tables/shard-{shard}/%s/%s%s",
				c.d.ClickHouse.DatabaseName(), table, c.d.ClickHouse.ZooKeeperSuffix())
		}
