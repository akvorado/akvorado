...
func (nd *Decoder) Decode(in decoder.RawFlow, _ decoder.Option, bf *schema.FlowMessage, finalize decoder.FinalizeFlowFunc) error {
	...
	if err := sflow.DecodeMessageVersion(buf, &packet); err != nil {
		...
		return fmt.Errorf("error while decoding sFlow: %w", err)
	}
	...
	return nd.decode(packet, bf, func() {
