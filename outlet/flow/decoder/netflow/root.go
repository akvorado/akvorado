...
func (nd *Decoder) Decode(in decoder.RawFlow, options decoder.Option, bf *schema.FlowMessage, finalize decoder.FinalizeFlowFunc) error {
	if len(in.Payload) < 2 {
	...
	case 5:
		...
		if err := netflowlegacy.DecodeMessage(buf, &packetNFv5); err != nil {
			...
			return fmt.Errorf("NetFlow v5 decoding error: %w", err)
		}
		versionStr = "5"
	...
	case 9:
		...
		if err := netflow.DecodeMessageNetFlow(buf, templates, &packetNFv9); err != nil {
			if !errors.Is(err, netflow.ErrorTemplateNotFound) {
				...
				return fmt.Errorf("NetFlow v9 decoding error: %w", err)
			}
	...
	default:
		...
		nd.metrics.stats.WithLabelValues(key, "unknown").
			Inc()
		return errors.New("unkown NetFlow version")
	}
