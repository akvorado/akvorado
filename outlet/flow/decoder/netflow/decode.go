...
func (nd *Decoder) decodeRecord(version uint16, obsDomainID uint32, samplingRateSys *samplingRateSystem, fields []netflow.DataField, ts, sysUptime uint64, options decoder.Option, bf *schema.FlowMessage, finalize decoder.FinalizeFlowFunc) {
	...
	for _, dir := range []direction{directionForward, directionReverse} {
		...
		for idx, field := range fields {
			...
			// Skip processing when field is not an IPFIX Reverse Information Element or we're looking in the forward direction
			if !ok || (field.PenProvided && (field.Pen != 29305 || dir == directionForward)) {
				continue
			}
			if field.PenProvided {
				foundReverseElement = true
			}
			shouldAddReversibleField := dir == directionForward || (dir == directionReverse && field.PenProvided)
			...
			case netflow.IPFIX_FIELD_sourceTransportPort:
				...
				if dir == directionForward {
					...
					bf.AppendUint(schema.ColumnDstPort, uint64(srcPort))
				}
			case netflow.IPFIX_FIELD_destinationTransportPort:
				...
				if dir == directionForward {
					...
					bf.AppendUint(schema.ColumnSrcPort, uint64(dstPort))
				}
			...
			case netflow.IPFIX_FIELD_ingressPhysicalInterface:
				if shouldAddReversibleField && bf.InIf == 0 {
					bf.InIf = uint32(decodeUNumber(v))
				}
			case netflow.IPFIX_FIELD_egressPhysicalInterface:
				if shouldAddReversibleField && bf.OutIf == 0 {
					bf.OutIf = uint32(decodeUNumber(v))
				}
		...
		if dir == directionForward && dataLinkFrameSectionIdx >= 0 {
		...
		// Necessary for keeping field priority the same as in the forward direction
		foundReverseInIf := bf.InIf != 0
		foundReverseOutIf := bf.InIf != 0

		// Use forward values (or sometimes reversed forward values) of some fields if reverse one is not present
